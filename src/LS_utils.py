...
def build_LS_json(df, text, doc_id, model_version = "0"):
...
def write_LS_json(df, text, doc_id, model_version = "0", out_folder = "LS_predictions", append = False):
    '''
    Write .json file for Label Studio from prediction DataFrame.
    Inputs:
        - df (DataFrame): DataFrame with predictions
        - text (string): text from pre_process()
        - doc_id (int): document id, used as name of the .json file
        - model_version (string): name of the model version
        - out_folder (string): path to output folder
        - append (bool): if True, add predictions to a previously existent .json file. Else, overwrite/create a new file
    '''
    if not append:
        file = build_LS_json(df, text, doc_id, model_version)
...
def merge_overlapping_models(df1, df2):
    '''
    Perform full join of predictions from 2 models.
    Inputs:
        - df1 (DataFrame): DataFrame with predictions from model 1
        - df2 (DataFrame): DataFrame with predictions from model 2
    Output:
        df1_ (DataFrame): merged predictions
    '''
    df1_ = df1.copy()
