...
# get locs from a text using the spacy NER model
...
# get locs from a text using the transformers xlm-roberta NER model
...
def get_spacy_predictions(df, model = None):
    '''
    Add Spacy location predictions.
    Inputs:
        - df (DataFrame): DataFrame from pre_process()
        - model (spacy model object): Spacy NER model to use. If model = None default is used:
            - "fr_core_news_md" if text language prediction is FR
            - "es_core_news_md" if text language prediction is ES
            - "en_core_web_md" if text doc language is EN or other
    Output:
        - df_ (DataFrame): DataFrame with location predictions
    '''
    df_ = df.copy()
    if model == None:
...
def get_transformer_predictions(df, model_path, model = None, tokenizer = None, mode = None):
    '''
    Add transformers location predictions.
    Inputs:
        - df (DataFrame): DataFrame from pre_process()
        - model_path (str): path to custom the model. Alternative to "model" parameter.
        - model (str): model name from https://huggingface.co/models. Alternative to "model_path"
        -tokenizer (str): tokenizer name. If None, default from "model_path" is used
        -mode (str): if "reload", reload model. Otherwise, use model in the environment
    Output:
        - df_ (DataFrame): DataFrame with location predictions
    '''
    df_ = df.copy()
