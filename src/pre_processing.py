...
def pre_processing(folder_path, lead_id, max_pages = 5):
    '''
    reshape .txt input from the PDF parser into a DataFrame.
    Split text pages and paragraphs (blocks).
    Input:
        - folder_path (string): path to the txt folder
        - lead_id (int): id of the document in folder_path
        - max_pages (int): max document pages to be fetched
    Output: 
        - df (DataFrame): text divided in pages and blocks, with offsets to text string  
